...
class Register1(Component):
  def __init__(self, name, loc, in_value, write_trigger, prev_read, read_trigger):
    ...
    mem = Memory(NodeType.MEM_FLIPFLOP, name + "_m", mem_loc, inputA=in_value, inputB=write_trigger)
    self._nodes.append(mem)

    and_loc = (loc[0], loc[1], loc[2]+2)
    and_relay = Relay(NodeType.RELAY_AND, name + "_a", and_loc, read_trigger, mem)
    self._nodes.append(and_relay)

    if prev_read is not None:
      or_relay = Relay(NodeType.RELAY_OR, name+"_o", loc, and_relay, prev_read)
